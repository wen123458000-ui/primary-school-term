...
def plot_results(results):
    ...
    ax1.bar(x - 1.5*width, results['bfs_wide'], width, label='BFS (宽浅型)', color='#3498db', alpha=0.8)
    ax1.bar(x - 0.5*width, results['dfs_wide'], width, label='DFS (宽浅型)', color='#e74c3c', alpha=0.8)
    ax1.bar(x + 0.5*width, results['bfs_deep'], width, label='BFS (窄深型)', color='#2ecc71', alpha=0.8)
    ax1.bar(x + 1.5*width, results['dfs_deep'], width, label='DFS (窄深型)', color='#f39c12', alpha=0.8)
    
    ax1.set_xlabel('节点数量')
    ax1.set_ylabel('时间 (毫秒)')
    ax1.set_title('BFS vs DFS 搜索性能对比')
    ax1.set_xticks(x)
    ...
    ax2 = axes[0, 1]
    ax2.plot(sizes, results['size_wide'], 'o-', label='宽浅型树', linewidth=2, markersize=8, color='#3498db')
    ax2.plot(sizes, results['size_deep'], 's-', label='窄深型树', linewidth=2, markersize=8, color='#e74c3c')
    ax2.set_xlabel('节点数量')
    ax2.set_ylabel('时间 (毫秒)')
    ax2.set_title('total_size() 递归计算性能')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    # 图3: find_duplicates() 性能
    ax3 = axes[1, 0]
    ax3.plot(sizes, results['dup_wide'], 'o-', label='宽浅型树', linewidth=2, markersize=8, color='#3498db')
    ax3.plot(sizes, results['dup_deep'], 's-', label='窄深型树', linewidth=2, markersize=8, color='#e74c3c')
    ax3.set_xlabel('节点数量')
    ax3.set_ylabel('时间 (毫秒)')
    ax3.set_title('find_duplicates() 查找重复项性能')
    ax3.legend()
    ...
    ax4.axhline(y=1.0, color='gray', linestyle='--', alpha=0.7, label='性能相等线')
    ax4.plot(sizes, ratio_wide, 'o-', label='BFS/DFS 时间比 (宽浅型)', linewidth=2, markersize=8, color='#9b59b6')
    ax4.plot(sizes, ratio_deep, 's-', label='BFS/DFS 时间比 (窄深型)', linewidth=2, markersize=8, color='#1abc9c')
    ax4.fill_between(sizes, 1, ratio_wide, alpha=0.2, color='#9b59b6')
    ax4.set_xlabel('节点数量')
    ax4.set_ylabel('BFS时间 / DFS时间')
    ax4.set_title('BFS vs DFS 相对性能 (>1表示DFS更快, <1表示BFS更快)')
    ax4.legend()
